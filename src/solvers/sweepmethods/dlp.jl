...
################################################################################
################# BOUNDARY-FUNCTION / WAVEFUNCTION SUPPORT ###################
################################################################################

"""
    symmetrize_layer_density(solver::DoubleLayerPotentialSolver, layer_density::AbstractVector, pts::BoundaryPoints, billiard::AbsBilliard) → full_density::Vector
...
## Arguments
* `solver`: The [`DoubleLayerPotentialSolver`](@ref) whose `symmetry` (if any) defines the folding.
...
"""
function symmetrize_layer_density(solver::DoubleLayerPotentialSolver, layer_density::AbstractVector{N}, pts::BoundaryPoints{T}, billiard::Bi) where {N<:Number,T<:Real,Bi<:AbsBilliard}
    Nfull = length(pts)
...
"""
    _dlp_adjoint_fredholm_matrix(solver::DoubleLayerPotentialSolver, pts::BoundaryPoints{T}, k::T; multithreaded::Bool = true) where {T<:Real} → A::Matrix{Complex{T}}

Assembles the weighted-transpose double-layer Fredholm matrix

    A(k) = I - W⁻¹D(k)ᵀW,   W = diag(ds),

whose (near-)null right singular vector is the boundary normal derivative
`u = ∂ₙψ` (see [`_dlp_boundary_function`](@ref)). This is a *bilinear*
weighted transpose (no complex conjugation), matching the same convention
`-develop`'s `adjoint_fredholm_matrix!` uses. When `solver.symmetry !== nothing`,
`W` uses the quadrature weights `ds` of the fundamental-domain representative
nodes and `A` has the same (reduced) size as [`construct_matrices`](@ref).

## Description
Rather than re-deriving the Kress/Bessel kernel a second time, `D(k)` is
recovered elementwise from the already-assembled primal Fredholm matrix
`F(k) = I - D(k)` returned by [`construct_matrices`](@ref) (`D = I - F`),
so this only costs an extra `O(n^2)` elementwise pass, not a second
special-function evaluation pass.

## Returns
*  `A` : The adjoint (weighted-transpose) Fredholm matrix, same size as `construct_matrices(solver, pts, k)`.
"""
function _dlp_adjoint_fredholm_matrix(solver::DoubleLayerPotentialSolver, pts::BoundaryPoints{T}, k::T; multithreaded::Bool=true) where {T<:Real}
    F = construct_matrices(solver, pts, k; multithreaded)
    n = size(F, 1)
    ds = pts.ds
    ds_idx = solver.symmetry === nothing ? (1:n) : symmetry_index_orbits(T, pts.xy, solver.symmetry).fundamental_indices
    A = similar(F)
    @inbounds for j in 1:n
        wj = ds[ds_idx[j]]
        for i in 1:n
            wi = ds[ds_idx[i]]
            Dji = i == j ? one(eltype(F)) - F[j, j] : -F[j, i]
            A[i, j] = -Dji * wj / wi
        end
    end
    @inbounds for i in 1:n
...
"""
    _dlp_boundary_function(solver::DoubleLayerPotentialSolver, billiard::AbsBilliard, k; multithreaded::Bool = true) → (u::Vector, pts::BoundaryPoints, norm::Real)

Computes the Rellich-normalized physical boundary normal derivative
`u = ∂ₙψ` of a double-layer-potential eigenstate at wavenumber `k`.

## Description
Unlike `state.vec` (the *primal* density from [`solve_vect`](@ref)), the
physical `∂ₙψ` is the nullspace of the *adjoint* (weighted-transpose)
Fredholm operator [`_dlp_adjoint_fredholm_matrix`](@ref), extracted via its
smallest right singular vector (`KrylovKit.svdsolve(A, 1, :SR)`, the same
convention [`solve_vect`](@ref) already uses for the primal problem). The
resulting fundamental-domain density (if `solver.symmetry !== nothing`) is
expanded onto the complete physical boundary with
[`symmetrize_layer_density`](@ref) before Rellich-normalization.

Shared by [`boundary_function`](@ref), [`momentum_function`](@ref),
[`husimi_function`](@ref) and [`wavefunction`](@ref) for [`BIMEigenstate`](@ref).

## Returns
*  `u` : The Rellich-normalized boundary normal derivative on the complete physical boundary.
*  `pts` : The complete-physical-boundary discretization corresponding to `u`.
*  `norm` : The Rellich-identity normalization applied to `u` (see [`_rellich`](@ref)).
"""
function _dlp_boundary_function(solver::DoubleLayerPotentialSolver, billiard::Bi, k; multithreaded::Bool=true) where {Bi<:AbsBilliard}
    T = _bim_numeric_type(solver)
    kT = T(k)
    pts = evaluate_points(solver, billiard, kT)
    A = _dlp_adjoint_fredholm_matrix(solver, pts, kT; multithreaded)
    @blas_1 _, _, rvecs, _ = KrylovKit.svdsolve(A, 1, :SR)
    u = symmetrize_layer_density(solver, Vector{Complex{T}}(rvecs[1]), pts, billiard)
    norm = _rellich(pts, u, kT)
    u = u ./ sqrt(norm)
    return u, pts, norm
end